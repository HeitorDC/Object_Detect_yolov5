            ...
            for *xyxy, conf, cls in reversed(det):
                ...
                for i in range(3):
                    for j in range(3):
                        ...
                        dist = dataset.depth_frame.get_distance(xcenter, ycenter)*1000
                        print("A distância é: " + str(dist))
                        Xtemp = dist*(xcenter - dataset.depth_intrin.ppx)/dataset.depth_intrin.fx
                        Ytemp = dist*(ycenter - dataset.depth_intrin.ppy)/dataset.depth_intrin.fy
                        Ztemp = dist
                        ...
                        distance = dist
                        print(cls, torch.tensor(xyxy))
                        print("X: ", Xtarget, ",", "Y: ", Ytarget, ",", "Z: ", Ztarget)
                        print("The distance of the object detected to the camera is: ", distance, "mm")
                        # coordinates_text = "(" + str(Decimal(str(Xtarget)).quantize(Decimal('0'), rounding=ROUND_HALF_UP)) + \", " + str(Decimal(str(Ytarget)).quantize(Decimal('0'), rounding=ROUND_HALF_UP)) + \", " + str(Decimal(str(Ztarget)).quantize(Decimal('0'), rounding=ROUND_HALF_UP)) + ")"
                        # cv2.putText(im0, text=coordinates_text, org=(int(interval_x)-160, int(interval_y)), fontFace=font, fontScale=3, color=(225, 255, 255), thickness=2, lineType=cv2.LINE_AA)
                        # text1 = "Xtemp : " + str(round(Xtemp))
                        # text2 = "Ytemp : " + str(round(Ytemp))
                        # text3 = "Ztemp : " + str(round(Ztemp))
                        ...
                        # cv2.putText(im0, text3, (int((xyxy[0] + xyxy[2])/2) - 40, int((xyxy[1] + xyxy[3])/2) + 80), cv2.FONT_HERSHEY_PLAIN, 3, [225, 255, 255], thickness=2, lineType=cv2.LINE_AA)
